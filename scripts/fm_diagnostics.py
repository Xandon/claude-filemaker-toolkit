...
Complexity Analysis Commands:
    complexity <name|id>       Deep complexity analysis of a single script
    complexity-report [n]      Ranked complexity report for all scripts (top N)
    complexity-html --output <file>  Generate interactive HTML dashboard
"""
...
def _calculate_cyclomatic_complexity(conn, script_id):
    """Count decision points: If, Else If, Exit Loop If, Case statements."""
    c = conn.cursor()
    c.execute("""
        SELECT COUNT(*) FROM script_steps ss
        WHERE ss.script_id = ? AND ss.enabled = 1
        AND (ss.step_name IN ('If', 'Else If', 'Exit Loop If')
             OR ss.human_readable LIKE '%Case(%')
    """, (script_id,))
    return c.fetchone()[0]


def _calculate_nesting_depth(conn, script_id):
    """Maximum nesting level of If/Loop blocks."""
    c = conn.cursor()
    c.execute("""
        SELECT step_index, step_name FROM script_steps
        WHERE script_id = ? AND enabled = 1
        AND step_name IN ('If', 'End If', 'Loop', 'End Loop')
        ORDER BY step_index
    """, (script_id,))
    ...
    for row in rows:
        if row['step_name'] in ('If', 'Loop'):
            current_depth += 1
            max_depth = max(max_depth, current_depth)
        elif row['step_name'] in ('End If', 'End Loop'):
            current_depth = max(0, current_depth - 1)

    return max_depth


def _analyze_variables(conn, script_id):
    """Count $local, $$global variables, potential naming conflicts."""
    c = conn.cursor()
    c.execute("""
        SELECT human_readable FROM script_steps
        WHERE script_id = ? AND enabled = 1
    """, (script_id,))
    ...
    # Potential conflicts: same name in both scopes
    conflicts = local_vars & global_vars

    return len(local_vars), len(global_vars), len(conflicts)


def _categorize_steps(conn, script_id):
    """Breakdown by category: flow control, data manipulation, navigation, UI, error handling."""
    c = conn.cursor()
    c.execute("""
        SELECT step_name FROM script_steps
        WHERE script_id = ? AND enabled = 1
    """, (script_id,))

    flow_control = {'If', 'Else If', 'Else', 'Exit Loop If', 'Loop', 'End Loop', 'End If', 'Go to Record', 'Exit Script', 'Halt Script', 'Pause/Resume Script'}
    data_manipulation = {'Set Field', 'Set Variable', 'Perform Find', 'Perform Find/Replace', 'Sort Records', 'Replace Field Contents', 'Delete Record/Request', 'Delete All Records', 'Duplicate Record/Request', 'Duplicate Set of Records', 'Import Records', 'Export Records'}
    navigation = {'Go to Layout', 'Go to Record', 'Go to Portal Row', 'Refresh Window', 'Toggle Window'}
    ui = {'Show Custom Dialog', 'Show/Hide Toolbars', 'Adjust Window', 'Move/Resize Window', 'Beep', 'Flush Cache'}
    error_handling = {'Set Error Capture', 'Get LastError'}

    categories = {'flow_control': 0, 'data_manipulation': 0, 'navigation': 0, 'ui': 0, 'error_handling': 0, 'other': 0}

    for row in c.fetchall():
        step_name = row['step_name'] or ""
        if step_name in flow_control:
            categories['flow_control'] += 1
        elif step_name in data_manipulation:
            categories['data_manipulation'] += 1
        elif step_name in navigation:
            categories['navigation'] += 1
        elif step_name in ui:
            categories['ui'] += 1
        elif 'Error' in step_name or 'error' in step_name:
...
def _assess_risk_indicators(conn, script_id):
    """Check for risky patterns and error handling."""
    c = conn.cursor()

    risks = []

    # Check if Set Error Capture is used and balanced
    c.execute("""
        SELECT COUNT(*) FROM script_steps
        WHERE script_id = ? AND enabled = 1
        AND human_readable LIKE '%Error Capture%On%'
    """, (script_id,))
    error_on = c.fetchone()[0]

    c.execute("""
        SELECT COUNT(*) FROM script_steps
        WHERE script_id = ? AND enabled = 1
        AND human_readable LIKE '%Error Capture%Off%'
    """, (script_id,))
    error_off = c.fetchone()[0]

    if error_on > 0 and error_off == 0:
        risks.append("⚠ Error Capture enabled but not disabled")

    # Risky operations without error handling nearby
    c.execute("""
        SELECT ss.step_index, ss.step_name FROM script_steps ss
        WHERE ss.script_id = ? AND ss.enabled = 1
        AND ss.step_name IN ('Perform Find', 'Delete Record/Request', 'Delete All Records', 'Import Records')
    """, (script_id,))

    risky_steps = c.fetchall()
    for risky in risky_steps:
        # Check if LastError is referenced within 5 steps after
        c.execute("""
            SELECT COUNT(*) FROM script_steps
            WHERE script_id = ? AND enabled = 1
            AND step_index > ? AND step_index <= ?
            AND human_readable LIKE '%LastError%'
        """, (script_id, risky['step_index'], risky['step_index'] + 5))
        if c.fetchone()[0] == 0:
            risks.append(f"⚠ {risky['step_name']} without nearby error check")

    # Loop complexity
    c.execute("""
        SELECT step_index FROM script_steps
        WHERE script_id = ? AND enabled = 1 AND step_name = 'Loop'
    """, (script_id,))

    loop_starts = [row['step_index'] for row in c.fetchall()]
    for loop_start in loop_starts:
        # Find corresponding End Loop
        c.execute("""
            SELECT step_index FROM script_steps
            WHERE script_id = ? AND enabled = 1
            AND step_name = 'End Loop' AND step_index > ?
            LIMIT 1
        """, (script_id, loop_start))
        loop_end_row = c.fetchone()
        if loop_end_row:
            loop_end = loop_end_row['step_index']
            c.execute("""
                SELECT COUNT(*) FROM script_steps
                WHERE script_id = ? AND enabled = 1
                AND step_index > ? AND step_index < ?
            """, (script_id, loop_start, loop_end))
            steps_in_loop = c.fetchone()[0]
            if steps_in_loop > 20:
                risks.append(f"⚠ Loop with {steps_in_loop} steps (complex logic)")

    # Layout switches inside loops
    c.execute("""
        SELECT COUNT(*) FROM script_steps ss
        WHERE ss.script_id = ? AND ss.enabled = 1
        AND ss.step_name = 'Go to Layout'
        AND EXISTS (
            SELECT 1 FROM script_steps loop_start
            WHERE loop_start.script_id = ss.script_id
            AND loop_start.step_name = 'Loop'
    ...
    """, (script_id,))
    if c.fetchone()[0] > 0:
        risks.append("⚠ Layout switch inside loop (performance critical)")
    ...
    c.execute("""
        ...
        ORDER BY step_index
    """, (script_id,))
    loop_indices = [row['step_index'] for row in c.fetchall()]
    for i, loop1 in enumerate(loop_indices):
        for loop2 in loop_indices[i+1:]:
            c.execute("""
                SELECT COUNT(*) FROM script_steps
                WHERE script_id = ? AND step_name = 'End Loop'
                AND step_index > ?
                ORDER BY step_index LIMIT 1
            """, (script_id, loop1))
            end_loop1 = c.fetchone()
            if end_loop1 and loop2 < end_loop1[0]:
                risks.append("⚠ Nested loops detected (high complexity)")
                break

    return risks


def _calculate_dependency_weight(conn, script_id, visited=None):
    """Total steps including all called subscripts (recursive)."""
    if visited is None:
        visited = set()
    ...
    c = conn.cursor()
    c.execute("SELECT step_count FROM scripts WHERE script_id = ?", (script_id,))
    row = c.fetchone()
    weight = row['step_count'] if row else 0

    # Add all called subscripts
    c.execute("""
        SELECT DISTINCT target_script_id FROM script_references
        WHERE source_type = 'script' AND source_id = ?
    """, (script_id,))

    for call in c.fetchall():
        weight += _calculate_dependency_weight(conn, call['target_script_id'], visited)

    return weight


def _calculate_maintainability_index(conn, script_id, complexity_metrics):
    """Composite score 0-100 based on complexity factors."""
    cyclomatic = complexity_metrics['cyclomatic']
    nesting = complexity_metrics['nesting_depth']
    steps = complexity_metrics['steps']
    dependency_weight = complexity_metrics['dependency_weight']

    # Base score: 100
    score = 100.0

    # Penalize for cyclomatic complexity (max penalty 25)
    score -= min(25, (cyclomatic / 40) * 25)

    # Penalize for nesting (max penalty 20)
    score -= min(20, (nesting / 6) * 20)

    # Penalize for step count (max penalty 30)
    score -= min(30, (steps / 150) * 30)

    # Penalize for dependency weight (max penalty 15)
    score -= min(15, (dependency_weight / 500) * 15)

    # Penalize for risks
    risks = complexity_metrics.get('risks', [])
    score -= min(10, len(risks) * 2)

    return max(0, min(100, score))


def _assign_complexity_grade(conn, script_id):
    """Calculate all metrics and assign A-F grade."""
    cyclomatic = _calculate_cyclomatic_complexity(conn, script_id)
    nesting = _calculate_nesting_depth(conn, script_id)
    ...
    dependency_weight = _calculate_dependency_weight(conn, script_id)
    risks = _assess_risk_indicators(conn, script_id)
    local_vars, global_vars, var_conflicts = _analyze_variables(conn, script_id)
    ...
    metrics = {
        ...
        'var_conflicts': var_conflicts
    }

    maintainability = _calculate_maintainability_index(conn, script_id, metrics)

    # Grade based on primary factors
    if cyclomatic <= 5 and nesting <= 2 and steps <= 30:
        grade = 'A'
    elif cyclomatic <= 10 and nesting <= 3 and steps <= 60:
        grade = 'B'
    elif cyclomatic <= 20 and nesting <= 4 and steps <= 100:
        grade = 'C'
    elif cyclomatic <= 35 and nesting <= 6 and steps <= 200:
        grade = 'D'
    else:
        grade = 'F'

    return grade, metrics, maintainability


def cmd_complexity(conn, name_or_id):
    """Deep complexity analysis of a single script."""
    script = find_script(conn, name_or_id)
    if not script:
        return
    ...
    grade, metrics, maintainability = _assign_complexity_grade(conn, script_id)

    print(f"\nComplexity Analysis: {script['name']}")
    print("=" * 80)
    print(f"Script ID: {script_id}  |  Total Steps: {script['step_count']}")
    print()
    ...
    print(f"  Dependency Weight:      {metrics['dependency_weight']} (includes subscript calls)")
    print(f"  Maintainability Index:  {maintainability:.1f}/100")
    print(f"  Grade:                  [{grade}]")
    print()
    ...
    print("─" * 80)
    print(f"  Local Variables ($):     {metrics['local_vars']}")
    print(f"  Global Variables ($$):   {metrics['global_vars']}")
    print(f"  Scope Conflicts:         {metrics['var_conflicts']}")
    print()

    # Step Type Distribution
    categories = _categorize_steps(conn, script_id)
    ...
    for cat, count in sorted(categories.items(), key=lambda x: -x[1]):
        if count > 0:
            ...
            print(f"  {cat.replace('_', ' ').title():<25} {count:>3}  ({pct:>5.1f}%)")
    print()
    ...
    # Script references
    c.execute("""
        SELECT COUNT(*) FROM script_references
        WHERE target_script_id = ?
    """, (script_id,))
    caller_count = c.fetchone()[0]

    c.execute("""
        SELECT COUNT(DISTINCT target_script_id) FROM script_references
        WHERE source_type = 'script' AND source_id = ?
    """, (script_id,))
    callee_count = c.fetchone()[0]

    print("DEPENDENCY ANALYSIS")
    print("─" * 80)
    print(f"  Called by {caller_count} other script(s)")
    print(f"  Calls {callee_count} subscript(s)")
    print()


def cmd_complexity_report(conn, top_n=20):
    """Ranked complexity report for all scripts with grades."""
    c = conn.cursor()

    # Collect all scripts with grades
    c.execute("""
        SELECT script_id, name, step_count FROM scripts
        WHERE is_folder = 0 AND step_count > 0
        ORDER BY script_id
    """)

    scripts_with_grades = []
    for row in c.fetchall():
        grade, metrics, maintainability = _assign_complexity_grade(conn, row['script_id'])
        scripts_with_grades.append({
    ...
    for s in scripts_with_grades:
        grade_counts[s['grade']] += 1

    print(f"\nScript Complexity Report")
    print("=" * 110)
    print(f"Total Scripts Analyzed: {len(scripts_with_grades)}")
    print()
    ...
    for grade in grades_order:
        ...
        pct = count / len(scripts_with_grades) * 100 if scripts_with_grades else 0
        bar = '█' * (count // 2) if count > 0 else ''
        print(f"  [{grade}] {count:>3} scripts  ({pct:>5.1f}%)  {bar}")
    print()

    # Top N most complex scripts
    scripts_with_grades.sort(key=lambda x: (x['grade'], -x['maintainability']))

    print(f"TOP {top_n} MOST COMPLEX SCRIPTS")
    print("─" * 110)
    print(f"{'Gr':>2} {'Cyclo':>6} {'Nest':>5} {'Steps':>6} {'DepWt':>6} {'Maint':>6} {'Risk':>5}  Name")
    print("─" * 110)

    for s in scripts_with_grades[:top_n]:
        print(f"[{s['grade']}] {s['cyclomatic']:>6} {s['nesting']:>5} {s['steps']:>6} "
              f"{s['dependency_weight']:>6} {s['maintainability']:>6.0f} {s['risks']:>5}  {s['name']}")

    print()

    # Complexity hotspots: complex AND heavily referenced
    c.execute("""
        SELECT target_script_id, COUNT(*) as caller_count
        FROM script_references
        GROUP BY target_script_id
    """)

    caller_counts = {row['target_script_id']: row['caller_count'] for row in c.fetchall()}

    hotspots = []
    for s in scripts_with_grades:
        if s['grade'] in ('D', 'F') and caller_counts.get(s['script_id'], 0) > 2:
            hotspots.append((s, caller_counts.get(s['script_id'], 0)))

    if hotspots:
        hotspots.sort(key=lambda x: -x[1])
        print("COMPLEXITY HOTSPOTS (Complex + Heavily Referenced)")
        print("─" * 110)
        print(f"{'Gr':>2} {'Callers':>8} {'Cyclo':>6} {'Nest':>5} {'Steps':>6}  Name")
        print("─" * 110)
        for s, caller_count in hotspots[:10]:
            print(f"[{s['grade']}] {caller_count:>8} {s['cyclomatic']:>6} {s['nesting']:>5} "
                  f"{s['steps']:>6}  {s['name']}")

    print()


def cmd_complexity_html(conn, output_path):
    """Generate an HTML complexity dashboard."""
    c = conn.cursor()
    ...
    for row in c.fetchall():
        ...
        categories = _categorize_steps(conn, row['script_id'])
    ...
    scripts_with_data.sort(key=lambda x: -x['maintainability'] if x['grade'] in ('D', 'F') else 100)
...
<head>
    ...
    <title>FileMaker Script Complexity Dashboard</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>FileMaker Script Complexity Dashboard</h1>
            <p style="color: #999; margin-top: 5px;">Analysis of script maintainability, cyclomatic complexity, and risk factors</p>
        </div>

        <div class="stats">
            <div class="stat-box">
                <div class="stat-number">{len(scripts_with_data)}</div>
                <div class="stat-label">Scripts</div>
            </div>
            <div class="stat-box">
                <div class="stat-number">{sum(s['steps'] for s in scripts_with_data)}</div>
                <div class="stat-label">Total Steps</div>
            </div>
            <div class="stat-box">
                <div class="stat-number">{sum(s['cyclomatic'] for s in scripts_with_data)}</div>
                <div class="stat-label">Total Complexity</div>
            </div>
            <div class="stat-box">
                <div class="stat-number">{sum(s['risks'] for s in scripts_with_data)}</div>
...
"""
    ...
    grades_order = ['A', 'B', 'C', 'D', 'F']
    max_count = max((grade_counts[g] for g in grades_order), default=1)

    for grade in grades_order:
        count = grade_counts[grade]
        pct = count / len(scripts_with_data) * 100 if scripts_with_data else 0
        height = (count / max_count * 180) if max_count > 0 else 0
        color = grade_colors[grade]
        html += f"""
                <div class="grade-bar">
                    <div style="display: flex; justify-content: center;">
                        <div class="bar" style="width: 60px; height: {height}px; background: {color};"></div>
                    </div>
...
"""
    ...
    html += """
        ...
        <div class="chart">
            <h2>Top 20 Most Complex Scripts</h2>
            <table>
                <thead>
                    <tr>
                        <th>Grade</th>
...
"""
    ...
    for s in scripts_with_data[:20]:
        color = grade_colors[s['grade']]
        html += f"""
                    <tr class="detail-row" onclick="this.classList.toggle('expanded')">
                        <td><span class="grade-badge" style="background: {color};">{s['grade']}</span></td>
                        <td>{s['name']}</td>
                        <td>{s['steps']}</td>
                        <td>{s['cyclomatic']}</td>
                        <td>{s['nesting']}</td>
                        <td>{s['dependency_weight']}</td>
                        <td>{s['maintainability']:.0f}/100</td>
                        <td>{s['risks']}</td>
                    </tr>
                    <tr class="detail-content">
                        <td colspan="8">
                            <strong>Step Distribution:</strong>
...
"""
        ...
        cat_colors = {
            'flow_control': '#8b5cf6',
            'data_manipulation': '#06b6d4',
            'navigation': '#f59e0b',
            'ui': '#ec4899',
            'error_handling': '#10b981',
            'other': '#6b7280'
        }

        for cat in ['flow_control', 'data_manipulation', 'navigation', 'ui', 'error_handling', 'other']:
            count = categories.get(cat, 0)
            if total_steps > 0 and count > 0:
                width = (count / total_steps) * 100
                label = cat.replace('_', ' ').title()
                html += f'<div class="category-segment" style="width: {width}%; background: {cat_colors[cat]};" title="{label}: {count}">{label} ({count})</div>'
...
"""
    ...
    <script>
        // Auto-expand complexity hotspots (D and F grades)
        document.querySelectorAll('tr.detail-row').forEach(row => {
            const gradeCell = row.querySelector('td:first-child').textContent.trim();
            if (gradeCell === 'D' || gradeCell === 'F') {
...
"""
    ...
    print(f"  Analyzed {len(scripts_with_data)} scripts")
...
def main():
    ...
    try:
        if command == 'hotspots':
            ...
            if not args:
                print("Usage: complexity <script_name_or_id>")
                sys.exit(1)
            cmd_complexity(conn, ' '.join(args))
        elif command == 'complexity-report':
            top_n = int(args[0]) if args else 20
            cmd_complexity_report(conn, top_n)
        elif command == 'complexity-html':
            if not args or '--output' not in args:
                print("Usage: complexity-html --output <filepath>")
                sys.exit(1)
            ...
            cmd_complexity_html(conn, output_path)
        else:
